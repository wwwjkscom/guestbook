...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="wwwjkscom/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/wwwjkscom/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@40.82.153.106 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no root@40.82.153.106 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
                                        -e MYSQL_IP=20.39.184.13 \
                                        -e MYSQL_PORT=3306 \
